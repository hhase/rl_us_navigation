...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_actions=5,
                 num_classes=10000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, noisy_layer=False):
        super(ResNet, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        self.noisy = noisy_layer
        self.inplanes = 64
        self.dilation = 1
        ...
        self.advantage = nn.Sequential(
            nn.Linear(num_classes, 1000, bias=False),
            nn.ReLU(),
    ...
    def get_state_action_values(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
